...
/**
 * Clase encargada de la Validacion de las cadenas originales a partir de
...
public class ValidaCadenas {
     ...
     *
     * @param cadenaOriginal parametro que se encarga de recibir la cadena
     * original a analizar
     * @return regresa un valor true en caso de que la cadena original coincida
     * con la expresion regular establecida para la validacion de cadena
     * original, regresara un valor false en caso contrario
     */
    public String validaCadenaOriginal(Object cadenaOriginal, byte[] BytesLlavePrivada, String password, boolean tipo) throws IOException, SecurityException, NoSuchProviderException, Exception {
        if (tipo == true) 
        {
            this.cadenaOriginal = cadenaOriginal;
            this.cadenaOriginal.toString();
            ...
            //if (match.matches()) {
                String aux = this.cadenaOriginal.toString();
                System.out.println("RECIBIDO LLAVE: "+BytesLlavePrivada);
                System.out.println("RECIBIDO PASS: "+password);
                PrivateKey LlavePrivada = Cifrado.getLlavePrivada(BytesLlavePrivada, password);
                String sello = Cifrado.firmar(LlavePrivada, aux.getBytes("UTF-8"));
