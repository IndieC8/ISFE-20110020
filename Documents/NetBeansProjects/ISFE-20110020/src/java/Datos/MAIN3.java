...
public class MAIN3 {
    public static void main(String[] args) throws InstantiationException, IllegalAccessException{
        try {
            ...
            /**File fiel=null;
            ResultSet rs=s.consulta("select archivoFiel from Fiel where idFiel=3;");
            java.sql.Blob blobFiel=null;
            while(rs.next()){
                blobFiel=rs.getBlob("archivoFiel");
                System.out.println("puntito"+blobFiel);
                //fiel=(File)blobFiel;
            }*/
            ...
            //    l=br.readLine();
            //}
            /**InputStream is=new FileInputStream(fiel);
            byte[] bFiel=new byte[(int)fiel.length()];
            int offset=0;
            int numRead=0;
            while(offset<bFiel.length && (numRead=is.read(bFiel, offset, bFiel.length-offset))>=0){
                offset+=numRead;
            }
            //System.out.println(blobFiel.length());
